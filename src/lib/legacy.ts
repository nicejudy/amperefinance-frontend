...
import { getProvider, getStaticProvider } from "./rpc";
...
export function useAccountDetails(overrideAccount) {
  const { library, account: connectedAccount } = useWeb3React();
  const active = true; // this is used in Actions.js so set active to always be true
  const account = overrideAccount || connectedAccount;
  ...
  const wmemoAddress = getContract(chainId, "WMEMO_ADDRESS");
  ...
  const key: any = shouldRequest ? [active, chainId, timeAddress, memoAddress, wmemoAddress, account] : false;
  const {
    data: details = {},
    ...
    dedupingInterval: 5000,
    fetcher: async (active, chainId, timeAddress, memoAddress, wmemoAddress, account) => {
      const provider = getProvider(library, chainId);
      const timeContract = new ethers.Contract(timeAddress, Token.abi, provider);
      const memoContract = new ethers.Contract(memoAddress, Token.abi, provider);
      ...
      try {
        const [memoBalance, timeBalance, wmemoBalance, stakeAllowance, unstakeAllowance, memoWmemoAllowance] = await Promise.all([
          ...
          getAllowance(timeContract, stakingHelperAddress),
          getAllowance(memoContract, stakingAddress),
          getAllowance(memoContract, wmemoAddress),
        ]);
        return {
          balances: {
            memo: memoBalance,
          ...
          staking: {
              time: stakeAllowance,
              memo: unstakeAllowance,
          },
          wrapping: {
              memo: memoWmemoAllowance,
          },
        };
      } catch (ex) {
        // eslint-disable-next-line no-console
  ...
  return [details, updateDetails, detailsError];
}

export function useCalculateUserBondDetails(bond, overrideAccount) {
  const { library, account: connectedAccount } = useWeb3React();
  const active = true; // this is used in Actions.js so set active to always be true
  const account = overrideAccount || connectedAccount;
  ...
  const key: any = shouldRequest ? [active, chainId, bond, account] : false;
  const {
    data: details = {},
    mutate: updateDetails,
    error: detailsError,
    ...
    dedupingInterval: 5000,
    fetcher: async (active, chainId, bond, account) => {
      if (!account) {
      ...
      const bondDetails = await bondContract.bondInfo(account);
...
export function useCalculateUserTokenDetails(token, overrideAccount) {
  const { library, account: connectedAccount } = useWeb3React();
  const active = true; // this is used in Actions.js so set active to always be true
  const account = overrideAccount || connectedAccount;
  ...
  const key: any = shouldRequest ? [active, chainId, token, account] : false;
  const {
    data: details = {},
    mutate: updateDetails,
    error: detailsError,
    ...
    dedupingInterval: 5000,
    fetcher: async (active, chainId, token, account) => {
      if (!account) {
        return new Promise<any>(resevle => {
          resevle({
            token: "",
      ...
      const tokenContract = new ethers.Contract(token.address, Mim.abi, provider);

      // allowance = await tokenContract.allowance(account, addresses.ZAPIN_ADDRESS);
...
export function useAppDetails() {
  ...
  const {
    ...
    dedupingInterval: 5000,
    fetcher: async (chainId, timeAddress, memoAddress, stakingAddress) => {
      const provider = getStaticProvider(chainId);
      // const mimPrice = await getTokenPrice("MIM");
      const timeContract = new ethers.Contract(timeAddress, Token.abi, provider);
      const memoContract = new ethers.Contract(memoAddress, Memo.abi, provider);
      const stakingContract = new ethers.Contract(stakingAddress, StakingContract.abi, provider);
...
export function useCalcBondDetails(bonds, value) {
  ...
  const key: any = [chainId, bonds, value];
  const {
    data: details = {},
    mutate: updateDetails,
    error: detailsError,
